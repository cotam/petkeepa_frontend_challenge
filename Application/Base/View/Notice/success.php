...
<body>
    <section class="section wide-fat notices">
        <div class="content-holder text-center">
			...
			<?php if (strpos($jumpUrl,'redirect_to=') !== false){substr($jumpUrl,strrpos($jumpUrl , '=' ) + 1);} ?>
			<?php if (strpos($jumpUrl,'redirect_to=') !== false) : ?>
				<?php if (substr($jumpUrl,strrpos($jumpUrl , '=' ) + 1) == '/') : ?>
					<h3><?php printf( _( 'This page will automatically <a id="href" href="%s">skip</a> in: <b id="wait">%s</b>' ), '/index.php/Home/Search/index.html', $waitSecond ); ?></h3>
				<?php else : ?>
					<h3><?php printf( _( 'This page will automatically <a id="href" href="%s">skip</a> in: <b id="wait">%s</b>' ), substr($jumpUrl,strrpos($jumpUrl , '=' ) + 1), $waitSecond ); ?></h3>
				<?php endif; ?>
			<?php else : ?>
